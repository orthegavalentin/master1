...
to find-notmates  ;; turtle procedure
  set notmates other turtles with  [color != [color] of myself] in-radius vision 
end
...
to find-obstacles ;; turtle procedure
  let obstacles patches with [pcolor = red] in-radius vision
  set obstacle min-one-of obstacles with [pcolor = red] [distance myself]
end
...
to-report vectNotAlign
  let x-component sum [sin (towards myself)] of notmates
  let y-component sum [cos (towards myself)] of notmates
  report (list x-component y-component)
end
...
to-report vectEvite
  ifelse is-patch? obstacle
    [
      let l (list (sin (towards obstacle + 180)) (cos (towards obstacle + 180)))
      report multiplyScalarvect (factor-evitement / distance obstacle) l
    ]
    [
      report (list 0 0)
    ]
