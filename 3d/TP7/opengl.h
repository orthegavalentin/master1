...
void drawSurfaceQuads(Point*** pts, long nbu, long nbv) {
	...
	glVertex3f(pts[nbu - 1][1]->getX(), pts[nbu-1][1]->getY(), pts[nbu-1][1]->getZ());
	glEnd();
}
...
Point*** sphere(Point* origin, int radius, int meridians, int parallels) {
	while((meridians / 2) % 2 != 1){meridians++;}
	while((parallels % 2 == 1) && (((parallels / 2)) % 2 != 1)) {parallels++;}

	std::cout << "meridians : " << meridians << std::endl;
	std::cout << "parallels : " << parallels << std::endl;
